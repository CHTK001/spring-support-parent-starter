...
import com.chua.rsocket.support.MsgEvent;
import com.chua.rsocket.support.MsgStep;
...
public interface RSocketSessionTemplate {
    ...
    /**
     * 保存或更新一个RSocket客户端的会话
     *
     * @param clientId 客户端唯一标识符
     * @param session  RSocket会话实例
     * @return 保存后的会话实例
     */
    RSocketSession save(String clientId, RSocketSession session);
     ...
     */
    void remove(String clientId, RSocketSession session);
     ...
     * @return 在线会话列表
     */
    List<RSocketSession> getOnlineSessions();
     ...
     * @return 在线用户列表
     */
    List<RSocketUser> getOnlineUsers(String type);

 ...
 */
@Data
@TableName(value = "monitor_sys_gen_table")
public class MonitorSysGenTable implements Serializable {
    /**
     * 表ID
     */
    ...
    @ColumnDesc("生成代码方式（0zip压缩包 1自定义路径）")
    private String tabGenType;

    /**
     * 生成路径（不填默认项目路径）
     */
    @ColumnDesc("生成路径（不填默认项目路径）")
     ...
     */
    @ColumnDesc("使用的模板（crud单表操作 tree树表操作 sub主子表操作）")
    @TableField(value = "tab_tpl_category")
    private String tabTplCategory;


    @TableField(exist = false)
    private MonitorSysGenColumn tabPkColumn;


    @TableField(exist = false)
    private List<MonitorSysGenColumn> tabColumns;
     ...
     * @param genProperties
     * @return {@link MonitorSysGenTable}
     */
    public static MonitorSysGenTable createSysGenTable(Integer genId,
                                                       String tableName,
                                                       Table tableResultSet,
                                                       ReportGenProperties genProperties) throws SQLException {
        MonitorSysGenTable sysGenTable = new MonitorSysGenTable();

 ...
 * @since 2024/6/21
 */
@SpiDescribe(value = "白名单")
public class ReportWhiteChainFilter implements Filter {
    ...
    public <T> void doFilter(ChainContext<T> context, FilterChain filterChain) {
        ...
        try {
            if(reportWhiteLimitFactory.tryAcquire(hostString) ) {
    ...
    private void doRegisterLog(String url, String hostString, String type) {
        POOL.execute(() -> {
            try {
                ...
                monitorProxyLimitLog.setMonitorProxyLogType("WHITE");

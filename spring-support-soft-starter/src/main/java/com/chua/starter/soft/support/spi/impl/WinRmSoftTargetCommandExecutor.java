...
@Component
public class WinRmSoftTargetCommandExecutor implements SoftTargetCommandExecutor {
    ...
    public SoftOperationResult execute(SoftTarget target, String command) throws Exception {
        WinRmExecClient client = new WinRmExecClient(toClientSetting(target, 5985));
        try {
            client.connect();
    ...
    public String readFile(SoftTarget target, String path) throws Exception {
        WinRmFileClient client = new WinRmFileClient(toClientSetting(target, 5985));
        try {
            client.connect();

...
public class IptablesServiceImpl implements IptablesService {
     ...
     * 构造方法，注入IP配置属�?
     *
     * @param ipProperties IP配置属性对象，包含数据库文件路径和IP解析类型等配置信�?
     */
     ...
     *                如果是包含端口的格式�?"192.168.1.1:8080"，会自动提取IP部分
     * @return 返回包含地理位置信息的封装结�?
     * 成功时返�? {@code ReturnResult.ok(GeoCity)} 包含城市、省份、国家等信息
     * 失败时返�? {@code ReturnResult.failed("错误信息")} 包含具体的错误原�?
     */
    @Override

 ...
 */
@Slf4j
@Spi("pay_unionpay_qr_code")
public class UnionPayQrCodeCreateOrderAdaptor extends WalletCreateOrderAdaptor {
    ...
    public ReturnResult<CreateOrderV2Response> createOrder(CreateOrderV2Request request, String userId, String openId) {
        ...
        try {
            ...
            return transactionTemplate.execute(it -> {
                PayMerchantOrder payMerchantOrder = createOrderObject(request, payUserWallet, openId, PayTradeType.PAY_UNIONPAY_QR_CODE, PayOrderStatus.PAY_CREATE);
                return createUnionPayOrder(payMerchantOrder);
            });
        } catch (Exception e) {
            log.error("[支付][创建订单]云闪付扫码支付创建订单失败", e);
    ...
    private ReturnResult<CreateOrderV2Response> createUnionPayOrder(PayMerchantOrder payMerchantOrder) {
        ...
        try {
            ...
            // UnionPayQrCodeRequest unionPayRequest = buildUnionPayQrCodeRequest(payMerchantOrder, config);
            // UnionPayQrCodeResponse response = unionPayClient.execute(unionPayRequest);
            
            // 临时实现：返回订单号和二维码URL，实际需要调用云闪付 API
            CreateOrderV2Response createOrderV2Response = new CreateOrderV2Response(payMerchantOrder.getPayMerchantOrderCode());
            // createOrderV2Response.setUrl(response.getQrCodeUrl());
            ...
            log.error("[支付][创建订单]云闪付扫码支付创建订单异常", e);

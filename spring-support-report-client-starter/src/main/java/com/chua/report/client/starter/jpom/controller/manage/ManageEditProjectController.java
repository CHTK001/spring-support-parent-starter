...
public class ManageEditProjectController extends BaseAgentController {
    ...
    public ManageEditProjectController(ClientWhitelistDirectoryService clientWhitelistDirectoryService,
    ...
    private void checkParameter(NodeProjectInfoModel projectInfo, boolean previewData) {
        ...
        if (runMode != RunMode.Link) {
            if (!previewData) {
                ...
                // 不是预检查数据才效验授权
                if (!clientWhitelistDirectoryService.checkProjectDirectory(whitelistDirectory)) {
                    if (outGivingProject) {
                        clientWhitelistDirectoryService.addProjectWhiteList(whitelistDirectory);
                    } else {
                ...
                if (StrUtil.isNotEmpty(logPath)) {
                    if (!clientWhitelistDirectoryService.checkProjectDirectory(logPath)) {
                        if (outGivingProject) {
                            clientWhitelistDirectoryService.addProjectWhiteList(logPath);
                        } else {

...
@Slf4j
public class ClientProjectFileBackupController extends BaseAgentController {

    private final ProjectFileBackupService projectFileBackupService;

    public ClientProjectFileBackupController(ProjectFileBackupService projectFileBackupService) {
    ...
    public void download(String id, @ValidatorItem(msg = "i18n.backup_id_missing.eaa2") String backupId, @ValidatorItem String filename, String levelName, HttpServletResponse response) {
        try {
            ...
            if (file.isDirectory()) {
                JakartaServletUtil.write(response, I18nMessageUtil.get("i18n.folder_download_not_supported.c3b7"), MediaType.TEXT_HTML_VALUE);
                return;
            }
            JakartaServletUtil.write(response, file);
        } catch (Exception e) {
            log.error(I18nMessageUtil.get("i18n.download_exception.e616"), e);
            JakartaServletUtil.write(response, I18nMessageUtil.get("i18n.download_file_error.5bcd") + e.getMessage(), MediaType.TEXT_HTML_VALUE);
        }
    }

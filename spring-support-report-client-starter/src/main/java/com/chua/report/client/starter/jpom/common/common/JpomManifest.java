 ...
 */
package com.chua.report.client.starter.jpom.common.common;
...
public class JpomManifest {
     ...
     * 当前运行类型
     */
    private final Type type = ClientJpomApplication.getAppType();
    /**
     * 系统名称
     */
    ...
    public static File getRunPath() {
        URL location = ClassUtil.getLocation(ClientJpomApplication.getAppClass());
    ...
    public static void releaseJar(String path, String version) {
        ...
        File to;
        while (true) {
            newFile = ClientJpomApplication.getAppType().name() + "-" + version + FileUtil.JAR_FILE_EXT;
    ...
    public static File getScriptFile() {
        ...
        String type = ClientJpomApplication.getAppType().name();
    ...
    public int getPort() {
        if (port == 0) {
            port = ClientJpomApplication.getInstance().getPort();
        }
        return port;
    }
    ...
    public String getDataPath() {
        if (StrUtil.isEmpty(dataPath)) {
            dataPath = ClientJpomApplication.getInstance().getDataPath();
        }
        return dataPath;
    }

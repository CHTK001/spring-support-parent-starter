...
@Slf4j
@RestController
@RequestMapping("/api/plugin/node-logger")
@RequiredArgsConstructor
public class NodeLoggerConfigController {
    ...
    public ResponseEntity<Map<String, Object>> getNodeLoggers(@PathVariable String nodeUrl) {
        try {
            ...
            List<PluginNodeLoggerConfig> loggers = nodeLoggerConfigService.getNodeLoggers(decodedNodeUrl);
    ...
    public ResponseEntity<Map<String, Object>> getLoggerConfig(
        ...
        try {
            ...
            Optional<PluginNodeLoggerConfig> config = nodeLoggerConfigService.getLoggerConfig(decodedNodeUrl, decodedLoggerName);
            Map<String, Object> details = nodeLoggerConfigService.getLoggerDetails(decodedNodeUrl, decodedLoggerName);
            ...
            response.put("availableLevels", PluginNodeLoggerConfig.getAllLogLevels());
    ...
    public ResponseEntity<Map<String, Object>> setLoggerLevel(
            @PathVariable String nodeUrl,
            @PathVariable String loggerName,
            @RequestParam PluginNodeLoggerConfig.LogLevel level) {
        ...
        try {
            ...
            boolean success = nodeLoggerConfigService.setLoggerLevel(decodedNodeUrl, decodedLoggerName, level);
    ...
    public ResponseEntity<Map<String, Object>> setLoggerLevelForAllNodes(
        ...
        try {
            ...
            Map<String, Boolean> results = nodeLoggerConfigService.setLoggerLevelForAllNodes(
                decodedAppName, decodedLoggerName, level);
            
            long successCount = results.values().stream().mapToLong(b -> b ? 1 : 0).sum();
            
            response.put("success", true);
            response.put("message", String.format("批量设置完成，成功: %d, 失败: %d", 
            ...
            log.error("Failed to set logger level for all nodes: {} - {} -> {}", 
                applicationName, loggerName, level, e);
    ...
    public ResponseEntity<Map<String, Object>> getNodesByApplicationName(@PathVariable String applicationName) {
        try {
            ...
            List<String> nodeUrls = nodeLoggerConfigService.getNodesByApplicationName(decodedAppName);
    ...
    public ResponseEntity<Map<String, Object>> refreshNodeLoggers(@PathVariable String nodeUrl) {
        ...
        try {
            ...
            boolean success = nodeLoggerConfigService.refreshNodeLoggers(decodedNodeUrl);
    ...
    public ResponseEntity<Map<String, Object>> getAllApplicationNames() {
        try {
            List<String> applicationNames = nodeLoggerConfigService.getAllApplicationNames();
    ...
    public ResponseEntity<Map<String, Object>> getAllLogLevels() {
        Map<String, Object> response = new HashMap<>();
        response.put("success", true);
        response.put("allLevels", PluginNodeLoggerConfig.getAllLogLevels());
        response.put("commonLevels", PluginNodeLoggerConfig.getCommonLogLevels());
    ...
    public ResponseEntity<Map<String, Object>> getLoggerStatistics() {
        try {
            ...
            statistics.put("totalApplications", nodeLoggerConfigService.getAllApplicationNames().size());

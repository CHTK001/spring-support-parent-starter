...
public class HelpCommand {
    ...
    public String help(@ShellOption(value = {"-c", "--command"}, defaultValue = "", help = "显示特定命令的帮助") String command) {
        ...
        if (command.isEmpty()) {
            // 显示总体帮助信息
            sb.append(helper.getColored("=== Spring Support Shell 帮助系统 ===", PromptColor.CYAN)).append("\n\n");

            sb.append(helper.getColored("系统命令:", PromptColor.YELLOW)).append("\n");
            sb.append("  help, h, ?           - 显示帮助信息\n");
            sb.append("  exit, quit           - 退出Shell\n");
            sb.append("  clear                - 清屏\n");
            sb.append("  history              - 显示命令历史\n\n");

            sb.append(helper.getColored("日志管理命令:", PromptColor.YELLOW)).append("\n");
            sb.append("  log-level            - 查看或修改日志等级\n");
            sb.append("  log-info             - 显示日志配置信息\n\n");
            
            sb.append(helper.getColored("登录统计命令:", PromptColor.YELLOW)).append("\n");
            sb.append("  login-stats          - 显示登录统计信息\n");
            sb.append("  login-history        - 显示登录历史记录\n");
            sb.append("  active-sessions      - 显示当前活跃会话\n\n");

            sb.append(helper.getColored("系统监控命令:", PromptColor.YELLOW)).append("\n");
            sb.append("  system-info          - 显示系统信息\n");
            sb.append("  memory-info          - 显示内存使用情况\n");
            sb.append("  thread-info          - 显示线程信息\n\n");

            sb.append(helper.getColored("使用提示:", PromptColor.GREEN)).append("\n");
    ...
    private String getCommandHelp(String command) {
        return switch (command.toLowerCase()) {
            case "log-level" -> helper.getColored("log-level 命令帮助:", PromptColor.CYAN) + "\n" +
            ...
            case "login-stats" -> helper.getColored("login-stats 命令帮助:", PromptColor.CYAN) + "\n" +
            ...
            case "system-info" -> helper.getColored("system-info 命令帮助:", PromptColor.CYAN) + "\n" +
                    "用法: system-info [type]\n" +
                    "参数:\n" +
                    "  type - 信息类型（可选）\n" +
            ...
            default -> helper.getColored("未知命令: " + command, PromptColor.RED) + "\n" +
                    "使用 'help' 查看所有可用命令";
        };
    }
    ...
    public String version() {
        String sb = helper.getColored("Spring Support Shell Starter", PromptColor.CYAN) + "\n" +
                "版本: 4.0.0.32\n" +
